...
public class JwtTokenAuthenticationFilter extends OncePerRequestFilter {
	...
	public JwtTokenAuthenticationFilter(WsProperties wsproperties) {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		String header = request.getHeader(wsproperties.getHeader());

		// 2. validate the header and check the prefix
		if (header == null || !header.startsWith(wsproperties.getPrefix())) {
		...
		// 3. Get the token
		String token = header.replace(wsproperties.getPrefix(), "");
		...
		try { // exceptions might be thrown in creating the claims if for example the token is
			...
			Claims claims = Jwts.parser()
					.setSigningKey(Base64Utils.encodeToString(wsproperties.getSecret().getBytes()).getBytes())
